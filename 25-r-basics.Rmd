...
```{remark}
Volleyball fans may be confused with the idea of spiking a ball from the 3-meter line, also called piping. 
So:
...
We use `col` to control the color,  `cex` (Character EXpansion) for the point size, and `abline` (ab is y=Ax+B) to add a straight line.
```{r, results='hold'}
...
toupper(x) # translate characters in character vectors from to upper case

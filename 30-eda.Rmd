...
```{r}
library(magrittr)
cbind(gender, drink) %>% head # bind vectors into matrix and inspect (`c` indicates Column)
...
table2.1
table.2.2 <- ftable(gender, drink, age) # A human readable table (`f` indicates Flat).
table.2.2
```
...
```{r}
prop.table(table1, margin = 1) # every row is summed up to 1
prop.table(table1, margin = 2) # every column amounts to 100%
```
...
- Zero observations are marked as a line. Negative obs. are more complicated and not discussed here.
...
When one of the variables is a (discrete) time variable, then the plot has a notion dynamics in time. For this see the Alluvian plot \@ref(alluvian).
...
For each (multivaraite) observation a simple univariate summary may be computed and visualized. These summaries may be compared, and the original (multivariate) observation inspected upon demand. 
Contact [Efrat](http://efratvil.github.io/home/index.html) for more details.  
![The Multinav Package](art/multinav.png)



An alternative approach starts with the a covariance matrix, $\hat \Sigma$, that can be visualized as an image.
...
## Mixed Type Data
Most real data sets will be of mixed type: both categorical and w. 

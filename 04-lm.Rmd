...
Denoting the dependent by $y$, the factors by $x$, and the effects by $\beta$ the linear model assumption implies that the expected response is the sum of the factors effects:

\begin{align}
  E[y]=\sum_{j=0}^p x_j \beta_j=x'\beta .
  (\#eq:linear-mean)
\end{align}
...
```{r, cache=TRUE}
twoWay <- read.csv('dataset_anova_twoWay_comparisons.csv')
head(twoWay)
```
...
```{r}
lm.2 <- lm(StressReduction~.-1,data=twoWay)
summary(lm.2)
```
Things to note:

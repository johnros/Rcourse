...
They may be correlated with themseslves; a positive departure from the model, may be followed by a series of positive departures etc.
Dignosing these _auto-correlations_ is a real art, which is not part of our course. 

The last assumption we required is normality. 
As previously stated, if $n \gg p$, this assumption is not realy needed. 
...
Judging from the figure, the normality assumption is quite plausable. 
...
If you are more familiar with the ANOVA literature, or that you don't want the effects of each level seperatly, but rather, the effect of __all__ the levels of each factor, use the `anova` command.
```{r}
anova(lm.2)
```
...
In the statistical parlance, this is called an _interaction_, or more persicely, an _interaction of order 2_.
```{r}
...
```
The syntax `Treatment * Age` means "mains effecs with second order interactions".
...
We might, but it is possible that no single interaction is significat, while thei combination is. 
To test for all the interactions together, we can simply check if the model without interactions is (significatly) better than a model with interactions. I.e., compare `lm.2` to `lm.3`.
This is done with the `anova` command.
```{r}
anova(lm.2,lm.3, test='F')
```
We see that `lm.3` is __not__ significatly better than `lm.2`, so that we can conclude that there are no interactions: different ages have the same response to different treatmetns.
...
- We used the `glht` fuction (generalized linear hypothesis test) from the package __multcompt__.

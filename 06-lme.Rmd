...
__If information of an effect is available at the time of prediction, treat it as a fixed effect. If it is not, treat it as a random-effect.__
...
In our diet example (\@ref(exm:random-effects))
...
- The output distinguishes between random effects, a source of variability, and fixed effect, which's coefficients we want to study. The mean of the random effect is not reported because it is unassumingly 0 for all random effects.

...
Also assume that tome subjects received some treatment, and other did not. 
When we want to infer on the population from which these subjects have been sampeled, we need to recall that some series of observations came from the same subject. 
If we were to ignore the subject of origin, and treat each observation as an independent sample point, we will think we have more information in our data than we actually have. 
...
In the simplest linear models of Chapter \@ref(lm), we thought of the variability as a measurement error, independent of anything else. This, however, is rarely the case when time or space is involved.
...
This is exactly the power of mixed-effect models: we specify the covariance via the sampling hierarchy.
...
groups <- gl(n = n.groups, k = n.repeats) # create a group factor
n <- length(groups)
z0 <- rnorm(10,0,10) # generate group effects
z <- z0[as.numeric(groups)] # generate random effects

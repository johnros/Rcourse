...
Sampes, $(x,y)$ have some distribution we denote $P$.
We want to learn a function that maps inputs to outputs. 
This function is called a _hypothesis_, and we denote it with $f:\mathcal{X} \to \mathcal{Y}$.
We also choose some other function that fines us for errorous prediction.
...
To make things more explicit, $f$ may be a linear function, and $l$ a squared error loss, in which case problem \@ref(risk) collapses to 
\begin{align}
  f^* := argmin_f \{ E_P[(x'\beta-y)^2] \}
\end{align}

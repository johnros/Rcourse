...
In our example: "what is the joint distribution of the $p$-measurements?"
...
### Simes' Test
A different approach to signal detection is justified by our fear of covariances. 
Indeed, estimating covariances is very difficult. That is why $T^2$ only works in low-dimensional problems. 
It would be nice if we could compute a vector of $p$ variable-wise p-values, ignoring covariances, and still be capable of valid signal detection.
One such algorithm, that allows statistically valid detection under a wide range of covariances (even if we don't know the covariance) is known as _Simes' Test_.
...
1. Return to the previous problem, but set `n=10`. Verify that you cannot compute your Hotelling statistic. 

Generate multivariate data with:
```{r}
mean<-c(0.2,3,4,0.1,6)
set.seed(3)
mean<-rexp(50,6)
multi<-  rmvnorm(n = 100, mean = mean) 
```
3. Use Hotelling's test to determine if μ equals $\mu_0=0$ in confidence of 95%. What is the P.value? 
    1. perform t.test on each variable and extract the P.value. Try to identify visually the variables which departed from $\mu_0$  in confidence of 95%.
    1. Use `p.adjust` function to determine in which variables there are any departures from $\mu_0=0$ with probability of more than 5%.
    1. Use `p.adjust` function to determine in which variables the proportion of departures is more than 5%.

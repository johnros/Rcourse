...
We start be generating some multivaraite data and computing the coordinate-wise (i.e. hypothesis-wise) p-value.
...
We now compute the pvalues of each coordiante.
...
We are now ready to do the identification, i.e., find which coordiante of $\mu$ is different than $\mu_0=0$.
The workflow is: 
...
Indeed- we are now able to detect that the first coordiantes carry signal, because their respective coordinate-wise null hypotheses have been rejected. 

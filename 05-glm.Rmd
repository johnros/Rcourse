...
The _odds_, is a scale of uncertitude defined as $$\frac{P(y=1)}{P(y=0)}.$$
```

```{definition, name="Odds Ratio"}
The _odds ratio_ is a measure of the difference between two binary random variables defined as the ratio between their odds. 
Formally:
...
Computing the OR between two conditions, indexed by $y|x=1$ and $y|x=0$, under the logistic link assumption returns:
\begin{align}
   OR(y|x=1,y|x=0) 
...
The last equality demystifies the choice of the link function in the logistic regression: __it allows us to interpret $\beta$ of the logistic regression as a measure of change of binary random variables, namely, the the odds-ratios (in log scale)__.

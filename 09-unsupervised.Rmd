...
This is the algebraic part of the problem, which is rather easy.
...
- __Variable__: 
A.k.a. _dimension_, or _feature_, or _column_. A measurement in its raw scale.

- __Feature Mapping__:
A.k.a. _variable transformation_, _data augmentation_. Measurements in some transformed scale.

- __Data__: 
A.k.a. _sample_, _observations_. 
Will typically consist of $n$, vectors of dimension $p$.
We typically denote the data as a $n\times p$ matrix $X$ from the space $\mathcal{X}$.

- __Network__: 
A mathematical representation of distances between points. We denote by $\mathcal{G}$.
...
- __Embedding__: 
Informally speaking: a "shape preserving" mapping of a space into another (see figure below). We denote an embedding of data $X$ into a manifold $\mathcal{M}$ by: $X \mapsto \mathcal{M}$.

- __Space Embedding Function__: 
An embedding may apply only to the observed data, $X$.
A _space embedding function_, $f$, allows to embed also unseen instances of $x$.
Formally: $f: \mathcal{X} \mapsto \mathcal{M}$.
...
It turns out that the PCA solution does not depend on the data itself, but only on the graph of Euclidean distances between data points ($\mathcal{G}=X'X$).
MDS can be thought of as a variation on PCA, that begins with this $n \times n$ graph of distances, and not the original $n \times p$ data matrix.
...
In particular, when the Euclidean distances implied by PCA are not inappropriate. 

Like PCA, and unlike MDS, local MDS includes both the construction of the dissimilarity/distance graph ($\mathcal{G}$), and the embedding ($X \mapsto \mathcal{M}$).  
Local MDS, as the name suggests, rests on the computation of $\mathcal{G}$ from _local_ distances.
The fundamental idea is the distance between two data points is the sum of the distances of closest points along the way. Put differently: distances are computed only between pairs of closest points, never in long ranges. 
Given the distances $\mathcal{G}$, local MDS ends with "regular MDS" for the embedding. 

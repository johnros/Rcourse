...
- The cluster assignemtn is given in the `cluster` element of the `stats::kmeans` output.
...
- K-medoids starts with the computation of a dissimiarity graph, done by the `dist` function.
- The clustering is done by the `cluster::pam` function.
- Ispecting the output confirms that similar states have been assigned to the same cluster. 
...
- We choose the sligle-linkage distance using the `method='single'` argument.

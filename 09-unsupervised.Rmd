...
Finding a linear combination, $v$, so that of $v'x$ best separates individuals.
...
Estimating all the PCs in the data is well defined algebraically if $n>p$ so viewing PCA as an exploratory tool, it is not a hard problem.
On the other hand, if viewing it as an inference tool, for inference on some population, then this is quite a hard problem. 
This is because recovering all the PCs is equivalent to estimating a covariance matrix. 
As we already saw in the Multivaraite Statistics Chapter (\@ref(multivariate)), estimating covariances is a hard task, we thus recommend: don't trust your PCs if $n$ is not much larger than $p$, and see the bibliographic notes for futher details. 
...
![Various embedding algorithms. No embedding of the sphere to the plane is perfect. This is obviously not new. [Maps makers](https://en.wikipedia.org/wiki/Map_projection) have known this for centuries!Source: http://scikit-learn.org/stable/auto_examples/manifold/plot_manifold_sphere.html#sphx-glr-auto-examples-manifold-plot-manifold-sphere-py](art/sphx_glr_plot_manifold_sphere_001.png)
...
MDS can be thought of as a variation on PCA, that begins with the $n \times n$ graph^ of distances between data points, and not the original $n \times p$ data.
The term _graph_ is typically used in this context, but saying _network_ instead of _graph_ is more appropriate. 
This is because a graph encodes connections (topology) and networks encode distances (geometry).

MDS aims at embedding a graph of distances into the plane, typically for visualization, while preserving the original distances.
...
It is an algorithm that compounds both the construction of the dissimilarity/distance graph, and the embedding. 
...
#### Kernel PCA (kPCA)
Returning to the BMI example (\@ref(exm:bmi)); what if we want to learn scores that best separate between individuals, but inlike PCA, are non-linear in the original features. 
...
Note that the _varimax_ rotation of factor-analysis has a similar goal: create factors with minimialy contributing variables, so that they are easy to explain. 
...
AH TODO: Something went wrong here with the plots:
...
## Practice Yourself
Read about the “Iris” dataset using `? iris`. Since we want to practice some unsupervised techniques, remove the `Species` column.

1. Make pairs scatter plot. Can you identify some clusters in the data? 
1. Perform K-means with `centers=3`. Perform Kmeans++. To extract the clustering results (cluster of each instance) use `kmeans$clusters`. Compare the accuracy of the two algorithms (use the species column from the original dataset). 
1. Perform hierarchal clustering with `hclust`, `method=”single”` and `method=”average”`.Extract the clustering results with `cutree`.  Compare the accuracy of the two linkage methods.

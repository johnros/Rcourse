...
Since the weights of Murder, Assault, and Rape are almost the same, and larger then UrbanPop, we conclude that PC1 captures the average crime rate in each state.
...
- For MDS, we first compute a dissimilarity graph with `dist`, and then learn the embedding with `cmdscale`.
...
Unlike the approaches in the Dimensionality Reduction Section \@ref(dim-reduce), it does not "compress" variables but rather observations. 
...
A delicate matter is that quantiles are not intuitive to define on multivariate random variables (lacking an ordering of the sapce), so that the "multivaraitre median", may be a more subtle quantity than you may think. See @small1990survey.
```
...
head(cut.2.2)   # printing the "clustering vector"
```

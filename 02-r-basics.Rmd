...
1. Save the numbers 1 to 100 into an object named `object`.
1. Write a loop that computes the mean of `object`.
1. Write a function that computes the mean of its input, and apply it on `object`.
1. Make a matrix of random numbers using `A <- matrix(rnorm(40), ncol=10, nrow=4)`. Compute the mean of each column. Do it using your own loop and then do the same with `lapply` or `apply`.
1. Make a data frame (`dataA`) with three columns. The first column with 100 numbers generated from the Normal distribution with mean=10 and sd=1 , second column with 100 numbers generated from the Poisson distribution with lambada=4. The third column contains `1`.  
Make another data frame (`dataB`) with three columns and 100 rows. Now with mean=10 and sd=0.5 , lambada=4 and `2`. 
1. Combine the two data frames into one with `dataAB <-rbind(dataA,dataB)`.
1. Make a scatter plot where the x-axes is the first column, the y-axes is the second and define the shape of the points to be the third column.
1. In a sample generated from the Normal distribution with mean=10 and sd=1. 
   1. What is the probability to have number little than 12.4?
   1. What is the number which is larger than 23% of the numbers?

...
1. Make a matrix of random numbers using `A <- matrix(rnorm(40), ncol=10, nrow=4)`. Compute the mean of each columns. Do it using your own loop and then do the same with `lapply` or `apply`.
1. Make a data frame (`dataA`) with three columns. The first with 100 numbers generated from Normal distribution with mean=10 and sd=1 , second with 100 numbers generated from Poisson distribution with lambada=4. The third contains `1`.  
...
1.In a sample generated from normal distribution with mean=10 and sd=1. 
   1. What is the probability to have number little then 12.4?

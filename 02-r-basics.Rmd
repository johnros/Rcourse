...
## Exctraction
R provides many ways to subset and extract elements from vectors and other objcets. 
...
```{r}
frame1[, 'sin'] %>% dim # extracts a column vector
frame1['sin'] %>% dim # extracts a data frame
frame1[,1:2] %>% dim # extracts a data frame
frame1[2] %>% dim # extracts a data frame
frame1[2, ] %>% dim # extract a data frame
frame1$sin %>% dim # extracts a column vector
```
...
If you want to learn moreabout subsetting see [Hadley's guide](http://adv-r.had.co.nz/Subsetting.html), and our Chapter \@ref(hadley)
...
- The output of `read.table` needs to be explcitly assigned to an object for it to be saved. 
...
This is very useful since it allows the filtering, selecting and joining operations to rely on the databased optimized algorithms.
...
A function in R, like everthing else, is a an object accesible using its name.
...
- The function `function` tells R to construc a function object.
- The arguments of the `function`, i.e. `(x,y)`, need to be named but we are not required to specify their type. 
- A typical R function does not change objects^[This is a classical _functional programming_ paradigm. If you are used to _object oriented_ programming, you may want to read about [references classes](http://adv-r.had.co.nz/R5.html) which may be required if you are planning to compute with very compilicated objects.] but rather creates new ones. 
...
Here is an embarasingly simple example
```{r}
for (i in 1:5){
    print(i)
    }
...
## Bibliographic Notes
There are endlesly many introductory texts on R.
I personally recommend the official inteoruction @venables2004introduction, or anything else Bill Venables writes. 
For advanced R pgoramming see @wickham2014advanced, or anything else Hadley Wickham writes. 

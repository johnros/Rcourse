...
A _compressed column oriented representation_, also known as _compressed sparse column_, or _CSC_, where the __column__ index is similar to COO, but instead of saving the row indexes, we save the locations in the colum index vectors where the row index has to increase. 
...
The `alpha` argument controls the type of regularization. Set to `alpha=0` for ridge, `alpha=1` for lasso, and any value in between for elasticnet regularization.
...
The best place to start reading on sparse representations and algorithms is the [vignettes](http://svitsrv=25.epfl.ch/R-doc/library/Matrix/doc/) of the __Matrix__ package. 

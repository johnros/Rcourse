...
- The `as(,"sparseMatrix")` function grows the dummy variable representation of the factor `x`. You will typically __not__ use this syntax, which is there only for demonstration.
...
A _compressed column oriented representation_, also known as _compressed sparse column_, or _CSC_, where the __column__ index is stored only when it changes. 
Put differently, in COO we store all non zero $(i,j,x_{i,j})$; in CSC we store $(i,j,x_{i,j})$ only when $j$ is incremented, and $(i,x_{i,j})$ otherwise. 
Clearly, the COO representation does not assume any order, and is thus non unique. 
CSC, on the other hand, does assume an ordering along columns, and rows within columns. 
...
A _compressed row oriented representation_, also known as _compressed sparse row_, or _CSR_, where the __row__ index is stored only when it changes. 
This is the same as CSC, only with an ordering along rows, and columns within rows.
...
There are provisions to also provide integer and complex (stored as double precision complex) matrices.
...
We bother with distinguishing between the different shapes of the matrix?
...
We now need implementations of our favorite statistical algorithms that make use of this represantation. 
At the time of writing, a very useful package that does that is the __glmnet__ package, which allows to fit linear models, generalized linear models, with ridge, lasso, and elsatic net regularization. 

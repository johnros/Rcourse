...
- R sessions/process may always be started using system calls. This creates _child processes_, or _spawn processes_ that cannot access the data and instructions of the calling process. 

- __Fork__: a mechanism, unique to Unix and Linux, that clones a process with accompanying instructions and data. All processes see the same memory in read-only state. Copies are made when data is changed by a process.


Inter-process communication standards:

- __Socket__: data sent via a network interface. Put differently- information is sent between R processes as if these were different machines in a network. Information may be structured by the particular application, and does not abide to some 
...
1. Decouple between our parallel algorithm and the parallelisation mechanism: we write parallelisable code once, and can later switch between paralliseation mechanisms. 
...
![top at 657\% CPU!](art/Screenshot from 2019-09-28 20-25-48.png)
...
### Caution: Implicit with Explicit Parallelism {#nested-parallel}
...
To understand _Spark_ need some backgroud.
The software that manages files on your disk is the [file system](https://en.wikipedia.org/wiki/File_system).
On personal computers, you may have have seens names like FAT32, NTFS, ext3, or others. 
Those are disk file systems. 
If your data is too big to be stored on a single disk, you may distibute it on several machines. 
...
Learning, which consistes of a lot iteratire algorithms run, requires fast and repeated data reads. 
This is the purpose of Spark.
Spark is a data oriented computing environemnt over distributed file systems. 
While origianlyl linked to HDFS, it may run over many other distributed file systems, but also locally, as we now demonstrate. 
...
# Spark performance
library(sparklyr)
spark_install(version = "2.1.0") # will download Spark on first run. 
library(dplyr)
...
- If data fits in RAM, you cannot beat `data.table`. But when your data gets too large to fit on disk, Spark is an excellent option. 
...
- `spark_connect` opens a connection to the Spark server.  
...
- When you have data in a connected Spark server, you can manage it using RStudio's [Connection pane](https://support.rstudio.com/hc/en-us/articles/115010915687-Using-RStudio-Connections).
- I have used the _dplyr_ syntax and not my favorite _data.table_ syntax. This is just how _sparklyr_ works.



### H2O {#h2o}

The machine learning library/algorithms in _Spark_ is called _Spark MLlib_.
It has many popular learing algorithms, and keeps updating.
One (of many) extension to _Spark_ is H2O, which replaces _Spark MLlib_, with many more learning algorithms.

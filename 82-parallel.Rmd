...
1. __Parallel R__: where the parallelism is managed with R.
1. __Parallel Extensions__: where R calls specialized libraries/routines/software that manage the parallelism themselves. 
...
- __Data Parallel:__ When performing different tasks on the same data. 
...
The __snow__ package was designed to parallelise using the _spawn_ mechanism, on all operating systems. 
Servers/R-sessions started with __snow__ will not see the parent's data, which will have to be copied to spawned sessions. 
...
1. Decouple between our parallel algorithm and the parallelisation mechanism: we write parallelisable code once, and can later switch between parallelisation mechanisms. 

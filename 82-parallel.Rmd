...
```{r}
airlines <- read.csv('data/2010_BSA_Carrier_PUF.csv')
airlines$ind <- sample(1:10, size = nrow(airlines), replace = TRUE)

timer <- function(i) max(airlines[airlines$ind==i,'CAR_LINE_SRVC_CNT'])
...
- The clear victor is _fork_: sessions start faster, and computations finish faster. Sadly, we recall that _forking_ is impossible on Windows machines, or between-machines in a cluster.
- We did not need to pass `airlines` to the different workers. `foreach` took care of that for us. 
...
```{r}
library(data.table)
airlines.dt <- as.data.table(airlines)
system.time(airlines.dt[,max(CAR_LINE_SRVC_CNT),ind])
```
No surprises there. 
If you can store your data on a single machine, `data.table` is still the fastest for operating on data subsets. 

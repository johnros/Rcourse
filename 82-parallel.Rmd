...
Machine learning is different than data munging. 
Most machine learning problems are actually optimization problems, that are harder to parallelise than _JOIN_, or _COUNT_ operations.
It is not recommended that you explicitely parallelise your learning algorithms. 
Luckily, many packages do that for you.

TODO: demonstrate package. 
...
Again- if data fits in RAM, you cannot beat `data.table`.



#### H2O {#h2o}

The learning library in _Spark_ is called _Spark MLlib_.
...
iris.hex <- as.h2o(iris) # load to the H2O server
h2o.ls() # check avaialbe data in the server
h2o.describe(iris.hex) # the H2O version of summary()
h2o.hist(iris.hex$Sepal.Length)

# Split to train and test
splits <- h2o.splitFrame(data = iris.hex, ratios = c(0.8))
train <- splits[[1]]
test <- splits[[2]]

...
The __readr__ package replaces base functions for importing and exporting data such as `read.table`. 
...
The above example involve `data.frame` objects, but __dplyr__ can handle other classes. 
In particular `data.table`s from the __data.table__ package, which is designed for very large data sets. 
...
This has the advantage that (a) you do not need to know the specific SQL implementation of your databse, and (b), you can enjoy the optimized algorithms provided by the database supplier. 

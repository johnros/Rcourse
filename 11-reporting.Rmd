...
It includes (i) an abstraction layer that specifies the layout of a web-site which is our report, (ii) the command to start a web server to deliver the site, and (iii) the interface between the web-server and an R session that feeds the data required for servicing.
For more on Shiny see @shiney.



In this book we will give a brief introduction to __knitr__ and give the references for further reading. 
...
### Markdown
Because __knitr__ builds upon _markdow_, here is a simple example of markdown text, to be used in a `.Rmd` file.
...
Underscores for _italics_, and double underscores for __bold__.

Math expressions are written in latex syntax.
```

The easiest way to compile is to click the __Knit__ button, just above the editing window. 


### Rmarkdown
_Rmarkdown_, is an extension of _markdown_ that allows to incorporate R expressions in the text, the will be avaluated at the time of compilation.
...
```{r qplot, fig.width = 5, fig.height = 5, fig.align = "center", eval=FALSE, verbatim = TRUE}
library(ggplot2)
qplot(wt, am, color = vs, data = mtcars)
```


You can also call r expressions inline. This is done with a single tick and the `r` argument.
For instance `r ''` ```{r label, what=ever} will output 

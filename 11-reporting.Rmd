...
For this purpose, Shiny uses the [Bootstrap](http://getbootstrap.com) web developpment technology.
...
You will probably want RStudio, which facilitatis the process.
...
The value of the `mytext` `inputId` can be queryied by the server using `input$mytext`.
...
- `shinyServer` is a (deprecatred) wrapper function for the server. 
...
This is the output.
```{r}
knitr::include_app('http://shiny.rstudio.com/gallery/example-01-hello.html')
```
...
This is the output:
```{r}
knitr::include_app('https://shiny.rstudio.com/gallery/tabsets.html')
```


### Augmenting Shiny

Here are several packages that augment __Shiny__:

- __shinyjs__ allows you to use common JavaScript operations in your Shiny applications. 
- __shinyURL__ allows you to save and restore the state of your shiny apps. 
...
For a Shiny manual, see @shiney, the [RStudio tutorial](http://shiny.rstudio.com/tutorial/), or [Zev Ross's](http://zevross.com/blog/2016/04/19/r-powered-web-applications-with-shiny-a-tutorial-and-cheat-sheet-with-40-example-apps/) excellent guide. 

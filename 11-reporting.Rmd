...
1. __Sweave__:
_Latex_ is a markup language that compiles to _Tex_ programs that compile to documents, typically PDFs.
...
There are many _markdown_ compilers. One of the most popular is _Pandoc_, written by the Berkeley philosopher(!) Jon MacFarlane. 
...
Image is the same as a link, starting with an exclamation, like this `![image title](image path)`.
...
For this purpose, Shiny uses the [Bootstrap](http://getbootstrap.com) web development technology.
...
To keep learning, see the RStudio's [tutorial](http://shiny.rstudio.com/tutorial/), and the Biblipgraphic notes. 
